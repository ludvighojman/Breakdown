...
Private Type TreeNode
    ...
    isRepeat As Boolean     ' The cell is listed again: not counted as a new dependent
...
' Which formula the formula card shows for a selected row. Dependents: the
...
Private Const FORMULA_FROM_ANCESTOR As Boolean = False
...
Private Sub UserForm_Initialize()
    ...
    ' Top bar: "Dependents of <cell>", the filter box, the sheet and an esc hint
    AddTextLabel("lblTopTitle", "Dependents of", TEXT_SIZE, False, ColorSecondary).AutoSize = True
    AddChip "lblRootChip"
            ...
            Else
                ...
                If nodeIndex > 0 Then
                    If NavigateToCell(NodeAddress(nodeIndex)) Then
                        RememberWindowSize
                        TraceHistory.Add Me
                        TraceUtils.ShowTraceDependents TraceHistory
                        Me.Hide
                    End If
                End If
            End If
...
' repeats (they add no new dependents), and repeat rows among them are copied
...
Private Sub TraceBack()
    ...
    Dim previousWindow As frmDependents
...
Private Sub ParseListBoxToTree()
    ...
    Dim i As Integer
    For i = 0 To lstDependents.ListCount - 1
        Dim addressText As String
        addressText = lstDependents.List(i, 0)
        ...
        hasChildren = False
        If i < lstDependents.ListCount - 1 Then
            Dim nextLevel As Integer
            nextLevel = GetIndentationLevel(lstDependents.List(i + 1, 0))
            hasChildren = (nextLevel > level)
        End If

        ' Store original data for this node
        Dim originalData As String
        originalData = lstDependents.List(i, 0) & "|" & _
                      lstDependents.List(i, 1) & "|" & _
                      lstDependents.List(i, 2)
...
Private Function NavigateToCell(dependentAddress As String) As Boolean
    On Error Resume Next
    ...
    If exclamationPosition > 0 Then
        ...
        If InStr(dependentAddress, "[") > 0 Then
        ...
        Else
            ...
            sheetName = Left(dependentAddress, exclamationPosition - 1)
        End If
    ...
    Else
        ...
        Set target = Range(dependentAddress)
    End If
    target.Select
...
' with the connecting reference as a green chip. Dependents: the parent's
...
Private Sub UpdateStatus()
    ...
    If Len(txtFilter.text) > 0 Then
        ...
        Me.Controls("lblStatus").Caption = " of " & UniqueCount & " dependents shown"
    Else
        Me.Controls("lblCount").Caption = CStr(UniqueCount)
        Me.Controls("lblStatus").Caption = IIf(UniqueCount = 1, " dependent", " dependents")
    End If
    LayoutFooterText
End Sub
...
' Put the "dependents" caption right after the count; both size to their text
